...
class MVPAConfig:
    ...
    def __init__(self):
        ...
        self.results_dir = Path(r"../../../results/searchlight_mvpa")
        ...
        # 特征选择参数
        self.apply_feature_selection = True  # 是否应用特征选择
        self.max_features = 1000  # 最大特征数
...
def apply_feature_selection(X, y, config, method='univariate', k=100):
    """应用特征选择"""
    if method == 'univariate':
        # 使用单变量特征选择
        selector = DynamicSelectKBest(score_func=f_classif, k=k)
        ...
        # 使用PCA降维
        reducer = DynamicPCA(n_components=k)
        X_reduced = reducer.fit_transform(X, y)
        
        # PCA没有特征索引，返回所有原始特征索引
...
def validate_classifier_performance(X, y, groups, config, sphere_info, contrast_name):
    """验证分类器性能"""
    try:
        # 构建pipeline
        ...
        if groups is not None and len(np.unique(groups)) > 1:
            cv = LeaveOneGroupOut()
            cv_scores = cross_val_score(pipeline, X, y, cv=cv, groups=groups, 
                                      scoring='accuracy', n_jobs=1)
        else:
            cv = StratifiedKFold(n_splits=min(config.cv_folds, len(y)), 
        ...
        return cv_scores, feature_ratio
        
    except Exception as e:
        log(f"分类器验证失败: {str(e)}", config)
        return np.array([0.5]), float('inf')
...
def run_permutation_test_debug(X, y, config, n_permutations=100):
    ...
    try:
        ...
        cv = StratifiedKFold(n_splits=min(config.cv_folds, len(y)), 
                           random_state=config.cv_random_state, shuffle=True)
        true_scores = cross_val_score(pipeline, X, y, cv=cv, scoring='accuracy', n_jobs=1)
        ...
        for i in range(n_permutations):
            ...
            perm_score = cross_val_score(pipeline, X, y_perm, cv=cv, scoring='accuracy', n_jobs=1)
            perm_scores.append(np.mean(perm_score))
        
        perm_scores = np.array(perm_scores)
        p_value = np.sum(perm_scores >= true_mean) / n_permutations
        
        return true_mean, perm_scores, p_value
        
    except Exception as e:
        log(f"置换测试失败: {str(e)}", config)
...
def prepare_classification_data(trial_info, beta_images, cond1, cond2, config):
    ...
    if np.any(selected_indices >= len(beta_images)) or np.any(selected_indices < 0):
        log(f"错误: 索引越界，索引范围: {selected_indices.min()}-{selected_indices.max()}, beta文件数: {len(beta_images)}", config)
        return None, None, None
