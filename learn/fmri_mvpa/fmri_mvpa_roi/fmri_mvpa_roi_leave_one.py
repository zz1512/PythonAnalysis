...
from sklearn.model_selection import cross_val_score, LeaveOneOut  # 改为LeaveOneOut
...
ROI_DIR = Path(r"H:\PythonAnalysis\learn_mvpa\ROI_masks")  # ROI掩码目录
...
def load_roi_masks(roi_dir):
    ...
    # 查找所有ROI文件
    roi_files = list(roi_dir.glob("ROI_*.nii.gz"))

    for roi_file in roi_files:
        roi_name = roi_file.stem.replace("ROI_", "")  # 从文件名提取ROI名称
        try:
...
def run_roi_classification(X, y, n_permutations=100):
    """在单个ROI上运行分类分析 - 使用留一法"""
    # 创建分类pipeline
    pipeline = Pipeline([
    ...
    for _ in range(n_permutations):
        ...
        perm_scores = cross_val_score(pipeline, X, y_perm, cv=loo, scoring='accuracy')
    ...
    return {
        ...
        'chance_level': 0.5,
        'n_folds': len(cv_scores)  # 留一法的折数等于样本数
    }
        ...
        for contrast_name, contrast_results in subject_results.items():
            for roi_name, roi_result in contrast_results.items():
                group_data.append({
                    ...
                    'n_folds': roi_result['n_folds']  # 添加折数信息
                })
...
def create_group_visualizations(group_df, contrasts):
    ...
    try:
        ...
        plt.title('ROI MVPA Classification Accuracy (Leave-One-Out)')  # 更新标题
        plt.tight_layout()
        ...
        for contrast_name, _, _ in contrasts:
            ...
            if not contrast_data.empty:
                ...
                plt.title(f'Classification Accuracy (Leave-One-Out): {contrast_name}')  # 更新标题
