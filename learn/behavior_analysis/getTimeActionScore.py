...
"""
=== 成对样本 t 检验（主分析，YY_mean vs KJ_mean） ===
被试数 n = 27
...
Cohen's dz = -0.189
Shapiro 正态性（差值）：W = 0.9658, p = 0.495581  (p>0.05 则差值近似正态)
"""
# ========= 配置区（按你的实际情况改这几项） =========
DATA_DIR = r"../../data_events"  # 28个文件所在的目录
OUT_DIR = r"../../result"  # 28个文件所在的目录
FILE_PATTERN = "*run-7_events.tsv"                    # 通配符，例：'*events.tsv' 或 '*events.csv'；默认两者都行
DV_COL = "action_time"                           # 你的记忆效果指标列名，比如 'accuracy' 或 'memory_score' 或 'rt'
YY_SET = {"yyw", "yyew"}
KJ_SET = {"kjw", "kjew"}
...
sub_df.to_csv(os.path.join(OUT_DIR, "run7_action_time.csv"), index=False, encoding="utf-8-sig")
...
print(f"\n已导出被试级均值表：{os.path.join(OUT_DIR, 'run7_action_time.csv')}")