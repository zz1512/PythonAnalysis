...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'F_jc_word.nii');     %%%%%%%
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    [number,~]=size(ds_shouci.samples);
    %% 
    data_weici=fullfile(sub_path,'F_jx_word.nii');     %%%%%%
...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'S_jc_word.nii');     %%%%%%%
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    [number,~]=size(ds_shouci.samples);
    %% 
    data_weici=fullfile(sub_path,'S_jx_word.nii');     %%%%%%
    ds_weici=cosmo_fmri_dataset(data_weici,'mask',mask_fn);
