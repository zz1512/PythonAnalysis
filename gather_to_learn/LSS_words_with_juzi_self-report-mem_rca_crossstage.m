...
run3_path='H:\metaphor\LSS\rsa\pattern_similarity_change_self-report_0\mvpa_pattern';  % F_yy_juzi.nii
run7_path='H:\metaphor\LSS\rsa\pattern_similarity_change_self-report_0\mvpa_pattern';   % S_yy_juzi.nii

mask_path='H:\metaphor\LSS'; 
out_path='H:\metaphor\LSS\words_with_juzi\self-report-mem\run3_with_run7';
%  can not be 6 11 16 19
for i_subj=1:nsubjects
    %%
    
    if i_subj == 6 || i_subj == 11 || i_subj == 16 || i_subj == 19
    ...
    all_ds_yy=cosmo_stack({ds_fyy,ds_syystart,ds_syyend});
   %all_ds_yy=cosmo_stack({all_ds_yy,ds_syyend});
    ...
    cosmo_map2fmri(result_yy, ...
        fullfile(output_path,'searchlight_yy_0.nii'));
    
end

%  can not be 6 11 16 19 24
for i_subj=1:nsubjects
    %%
    
    if i_subj == 6 || i_subj == 11 || i_subj == 16 || i_subj == 19 || i_subj == 24
   ...
   all_ds_kj=cosmo_stack({all_ds_kj,ds_Skjend});
    ...
    cosmo_map2fmri(r_kj, ...
        fullfile(output_path,'searchlight_kj_0.nii'));
