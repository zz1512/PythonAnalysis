...
run4_path='H:\metaphor\First_level\single_trial_v1\pattern';   % S_yy_juzi.nii

mask_path='H:\metaphor\First_level\single_trial_v1'; 
out_path='H:\metaphor\cross_similarity\run3_and_4\yy_and_kj';
...
for i_subj=1:nsubjects
    ...
    sub_4_path=fullfile(run4_path,subject_id);
    %%
    ...
    %%%
    Syy=fullfile(sub_4_path,'S_yy_juzi.nii');
    ds_syy=cosmo_fmri_dataset(Syy,'mask',mask_fn);
 
    all_ds_yy=cosmo_stack({ds_fyy,ds_syy});
   ...
   Skj=fullfile(sub_4_path,'S_kj_juzi.nii');
   ds_Skj=cosmo_fmri_dataset(Skj,'mask',mask_fn);
   all_ds_kj=cosmo_stack({ds_Fkj,ds_Skj});
    measure = @new_measure_dsm_corr;
    voxel_count=100;
