 ...
 %   myexplained_lsc=0;
  ...
  %  RD_eig_hsc=0;
   ...
   % myexplained_hsc=0;
    ...
    %%  HSC组，计算3个指标
    F_yy_s = fullfile(sub_path,'run7_yy_start_word.nii');
    F_yy_s_data = cosmo_fmri_dataset(F_yy_s,'mask',mask_fn);
    
    F_yy_e = fullfile(sub_path,'run7_yy_end_word.nii');
...
% 
[h,p,ci,stats]=ttest(RDrun7,RDjuzi)
counterp = counterp + 1;
allp(counterp,:) = p;
alldata = [alldata,RDrun7,RDjuzi];
end

save lHPC_yy3_vs_juzi1.mat allp alldata
