...
for s = 1:length(subjects)
    ...
    %% 
    data_f=fullfile(sub_path,'F_kj_sword.nii');     %%%%%%%
    ds_f=cosmo_fmri_dataset(data_f,'mask',mask_fn);
    [number,~]=size(ds_f.samples);
    %% 
    data_s=fullfile(sub_path,'S_kj_sword.nii');     %%%%%%
...
for s = 1:length(subjects)
    ...
    %% 
    data_f=fullfile(sub_path,'F_kj_eword.nii');     %%%%%%%
    ds_f=cosmo_fmri_dataset(data_f,'mask',mask_fn);
    [number,~]=size(ds_f.samples);
    %% 
    data_s=fullfile(sub_path,'S_kj_eword.nii');     %%%%%%
    ds_s=cosmo_fmri_dataset(data_s,'mask',mask_fn);
