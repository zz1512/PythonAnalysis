...
for s = 1:length(subjects) 
   ...
   mask_fn = fullfile(mask_path,'gm.nii');
    ...
    %% 
    data_q=fullfile(sub_path,'F_kj_start_word.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'F_kj_end_word.nii');  
...
for s = 1:length(subjects) 
    ...
    %% 
    data_q=fullfile(sub_path,'S_kj_start_word.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'S_kj_end_word.nii');  
    ds_h=cosmo_fmri_dataset(data_h,'mask',mask_fn);
    %%
    all_ds = cosmo_stack({ds_q,ds_h});
    measure = @kernel_measure;
