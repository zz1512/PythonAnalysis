...
for s = 1:length(subjects)
    ...
    %%  run7 kj
    S_kj_s = fullfile(sub_path,'S_kj_start_word.nii');
    S_kj_s_data = cosmo_fmri_dataset(S_kj_s,'mask',mask_fn);
    
    S_kj_e = fullfile(sub_path,'S_kj_end_word.nii');
...
%    r_hsc = cosmo_corr(rec_dsm(:), predict_dsm(:), 'Spearman');
   ...
   %% yy   
   %%  run7 yy
    S_kj_s = fullfile(sub_path,'S_yy_start_word.nii');
    S_kj_s_data = cosmo_fmri_dataset(S_kj_s,'mask',mask_fn);
    
    S_kj_e = fullfile(sub_path,'S_yy_end_word.nii');
    S_kj_e_data = cosmo_fmri_dataset(S_kj_e,'mask',mask_fn);
