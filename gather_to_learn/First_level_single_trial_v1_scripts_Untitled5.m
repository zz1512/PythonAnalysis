...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'F_kj_eword.nii');    
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    beta =mean(mean(ds_shouci.samples,2));
    counter = counter+1;
    F_kj_eword(counter,:) = beta;  
...
mean(S_kj_sword)
mean(S_kj_eword)
F = F_kj_sword+F_kj_eword;
S = S_kj_sword+S_kj_eword;
