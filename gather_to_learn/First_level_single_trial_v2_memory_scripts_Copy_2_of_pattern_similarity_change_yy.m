...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'First_F_yy_start_word.nii');     %%%%%%%
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    [number,~]=size(ds_shouci.samples);
    %% 
    data_weici=fullfile(sub_path,'First_F_yy_end_word.nii');     %%%%%%
...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'Second_F_yy_start_word.nii');     %%%%%%%
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    [number,~]=size(ds_shouci.samples);
    %% 
    data_weici=fullfile(sub_path,'Second_F_yy_end_word.nii');     %%%%%%
    ds_weici=cosmo_fmri_dataset(data_weici,'mask',mask_fn);
