 ...
 %   myexplained_lsc=0;
  ...
  %  RD_eig_hsc=0;
   ...
   % myexplained_hsc=0;
    ...
    %%  HSC组，计算3个指标
    F_yy_s = fullfile(sub_path,'F_kj_start_word.nii');
    F_yy_s_data = cosmo_fmri_dataset(F_yy_s,'mask',mask_fn);
    
    F_yy_e = fullfile(sub_path,'F_kj_end_word.nii');
...
%    RD_eff_hsc=100*RD_eig_hsc/myexplained_hsc;
 ...
 %   RDeff_hsc(counter,:)=RD_eff_hsc;
    %% LSC组
    S_yy_s = fullfile(sub_path,'run7_kj_start_word.nii');
    S_yy_s_data = cosmo_fmri_dataset(S_yy_s,'mask',mask_fn);
    
    S_yy_e = fullfile(sub_path,'run7_kj_end_word.nii');
...
%     end
 ...
 %   RDeig_lsc(counter1,:)=RD_eig_lsc;
    RDrun7(counter1,:)=RD_var_yy;
 %   RDeff_lsc(counter1,:)=RD_eff_lsc;
end
% 
[h,p,ci,stats]=ttest(RDF,RDrun7)
counterp = counterp + 1;
allp(counterp,:) = p;
alldata = [alldata,RDF,RDrun7];
end

save kj3_vs_kj1_lHPC.mat allp alldata
