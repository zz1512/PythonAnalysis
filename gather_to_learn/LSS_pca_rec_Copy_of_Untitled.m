% recontruct with first leading pc
clear;clc;
subjects = {'sub-01';'sub-02';'sub-03';'sub-04';'sub-05';'sub-06';'sub-07';'sub-08';'sub-09';'sub-10';'sub-11';'sub-13';'sub-14';'sub-15';'sub-16';'sub-17';'sub-18';'sub-19';'sub-20';'sub-21';'sub-22';'sub-24';'sub-25';'sub-26';'sub-27';'sub-28'};
masks = {'Hippocampus_L.nii'}; % Left_Hippocampus_harvard_oxford;; Hippocampus_L;;aHPC_L;Hippocampus_L_R
study_path='H:\metaphor\LSS\rsa\pattern_similarity_change\mvpa_pattern';
roi_path = 'H:\metaphor\mask'; 
n_subjects = numel(subjects);
msk = masks{1};
pcpc = 35;
counter = 0;

for s = 1:length(subjects)
    
    sub = subjects{s};
    sub_path=fullfile(study_path,sub);
    mask_fn=fullfile(roi_path,msk);
   
    
    %  run7 kj
    S_kj_s = fullfile(sub_path,'run7_kj_start_word.nii');
    S_kj_s_data = cosmo_fmri_dataset(S_kj_s,'mask',mask_fn);
    
    S_kj_e = fullfile(sub_path,'run7_kj_end_word.nii');
    S_kj_e_data = cosmo_fmri_dataset(S_kj_e,'mask',mask_fn);
    
    run7=(S_kj_s_data.samples + S_kj_e_data.samples)*0.5;
   
    predict = run7'; 
    % juzi1 kj
    syyjuzi = fullfile(sub_path,'F_kj_juzi.nii');
    syyjuzi = cosmo_fmri_dataset(syyjuzi,'mask',mask_fn);
    juzi_kj = syyjuzi.samples'; 
    
    %
    [coeff,score,~,~,explained,mu]=pca(juzi_kj);
     
  
   num_pcs = pcpc;
   top_pcs = coeff(:,1:num_pcs);
   %full_rec = score * coeff'+ mu;
   reconstructed_data = score(:, 1:num_pcs) * top_pcs'+mu;


   
%    predict_dsm = cosmo_pdist(predict', 'correlation');
%    rec_dsm = cosmo_pdist(reconstructed_data', 'correlation');
%    r_hsc = cosmo_corr(rec_dsm(:), predict_dsm(:), 'Spearman');
   %
   rkj = 0;
   
   for x = 1:size(juzi_kj,2)
       
         r = corr(predict(:,x),reconstructed_data(:,x),'type','Spearman');
         rkj= rkj + atanh(r);
   end

   % yy   
   %  run7 yy
    S_kj_s = fullfile(sub_path,'run7_yy_start_word.nii');
    S_kj_s_data = cosmo_fmri_dataset(S_kj_s,'mask',mask_fn);
    
    S_kj_e = fullfile(sub_path,'run7_yy_end_word.nii');
    S_kj_e_data = cosmo_fmri_dataset(S_kj_e,'mask',mask_fn);
    
    run7=(S_kj_s_data.samples + S_kj_e_data.samples)*0.5;
   
    predict = run7'; 
    % juzi1 kj
    syyjuzi = fullfile(sub_path,'F_yy_juzi.nii');
    syyjuzi = cosmo_fmri_dataset(syyjuzi,'mask',mask_fn);
    juzi_yy = syyjuzi.samples'; 
    
    [coeff,score,~,~,explained,mu]=pca(juzi_yy);
   
   num_pcs = pcpc;
   
   top_pcs = coeff(:,1:num_pcs);
   full_rec = score * coeff'+ mu;
   reconstructed_data = score(:, 1:num_pcs) * top_pcs'+mu;

  % predict = juzi_yy;
  % [A,B,rr,U,V] = canoncorr(predict,reconstructed_data);
   %
   ryy = 0;
   
   for x = 1:size(juzi_yy,2)
        %  r = 1 - pdist2(predict(x,:), reconstructed_data(x,:),'cosine');
%         r = corr(predict(:,x),reconstructed_data(:,x));
         r = corr(predict(:,x),reconstructed_data(:,x),'type','Spearman');
         ryy= ryy + atanh(r);
   end
   
   counter = counter + 1;
    kj(counter,:) = rkj;
    yy(counter,:) = ryy;  

end

[h,p,ci,stats] = ttest(kj,yy)

yy(11)=[];
kj(11)=[];
load H:\metaphor\events_true_memory\true_memory_yy_no_sub11_12_23.mat
load H:\metaphor\events_true_memory\true_memory_kj_no_sub11_12_23.mat
[r1,p1] = corr(yy,memory_yy,'type','Spearman')
[r2,p2] = corr(kj,memory_kj,'type','Spearman')

%[r3,p3] = corr(yy-kj,memory_yy-memory_kj,'type','Spearman')

% load H:\metaphor\events_memory\selfreportmemory_yy_no_sub11_12_23.mat
% load H:\metaphor\events_memory\selfreportmemory_kj_no_sub11_12_23.mat
% 
% [r4,p4] = corr(yy,memory_yy,'type','Spearman')
% [r5,p5] = corr(kj,memory_kj,'type','Spearman')
% [r6,p6] = corr(yy-kj,memory_yy-memory_kj,'type','Spearman')
