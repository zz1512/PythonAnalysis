...
measure = @kernel_measure_from_F_to_S;
voxel_count = 100;
%% first yy 
for s = 1:length(subjects) 
    ...
    %% 
    data_q=fullfile(sub_path,'F_yy_sword.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'S_yy_sword.nii');  
    ...
    cosmo_map2fmri(result, ...
        fullfile(output_path,'yy_sword_from_F_to_S.nii'));
end
...
for s = 1:length(subjects) 
    ...
    %% 
    data_q=fullfile(sub_path,'F_yy_eword.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'S_yy_eword.nii');  
    ds_h=cosmo_fmri_dataset(data_h,'mask',mask_fn);
    %%
    all_ds = cosmo_stack({ds_q,ds_h});
    ...
    result = cosmo_searchlight(all_ds,nbrhood,measure);
    cosmo_map2fmri(result, ...
        fullfile(output_path,'yy_eword_from_F_to_S.nii'));
end
...
for s = 1:length(subjects) 
    ...
    %% 
    data_q=fullfile(sub_path,'F_jx_word.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'S_jx_word.nii');  
    ds_h=cosmo_fmri_dataset(data_h,'mask',mask_fn);
    %%
    all_ds = cosmo_stack({ds_q,ds_h});
    ...
    result = cosmo_searchlight(all_ds,nbrhood,measure);
    cosmo_map2fmri(result, ...
        fullfile(output_path,'jx_word_from_F_to_S.nii'));
end