...
out_path = 'H:\metaphor\LSS\rsa\pattern_similarity_change\searchlight';
...
for s = 1:length(subjects) 
    ...
    %% 
    data_q=fullfile(sub_path,'run7_yy_start_word.nii');  
    ds_q=cosmo_fmri_dataset(data_q,'mask',mask_fn);
    data_h=fullfile(sub_path,'run7_yy_end_word.nii');  
    ...
    cosmo_map2fmri(result, ...
        fullfile(output_path,'yy_run7.nii'));
end
