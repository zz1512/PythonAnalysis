...
%     %% 
%     data_weici=fullfile(sub_path,'S_kj_eword.nii');     %%%%%%
...
for s = 1:length(subjects)
    ...
    %% 
    data_shouci=fullfile(sub_path,'M_kj_sword.nii');     %%%%%%%
    ds_shouci=cosmo_fmri_dataset(data_shouci,'mask',mask_fn);
    [number,~]=size(ds_shouci.samples);
    %% 
    data_weici=fullfile(sub_path,'M_kj_eword.nii');     %%%%%%
    ds_weici=cosmo_fmri_dataset(data_weici,'mask',mask_fn);
    ...
    counter = counter+1;
    my_r_M(counter,:) = allr/number;  
    
end

%%
% [h,p,ci,stats] = ttest(my_r_F,my_r_M)
% mean(my_r_F) 
mean(my_r_M)
