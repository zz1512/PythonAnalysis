...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage 21: hippocampal seed PCA-connectivity searchlight (MATLAB Dimension story)."
    )
    parser.add_argument(
        "--config-json",
        type=Path,
    ...
    parser.add_argument("--seed-mask-path", type=Path, default=None)
...
def main() -> None:
    ...
    args.seed_mask_path = pick_path(
        cli_value=args.seed_mask_path,
        config_payload=config,
    ...
    args.output_dir = pick_path(
        ...
        preset_value=output_root / "story_connectivity_suite",
    )
    ...
    logging.info("Stage 21 finished: %s", summary["group_summary"])
