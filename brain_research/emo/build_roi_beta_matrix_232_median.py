...
def extract_roi_medians(beta_data: np.ndarray, roi_indices: List[np.ndarray]) -> np.ndarray:
    """按 ROI 索引计算中位数。"""
    return np.array([np.nanmedian(beta_data[idx]) for idx in roi_indices], dtype=np.float32)
...
def extract_space_matrix(
    ...
    for si, s in enumerate(subjects):
        for ki, stim in enumerate(stimuli):
            ...
            mat[:, si, ki] = extract_roi_medians(data, roi_indices)
...
def main() -> None:
    ...
    # 3. 提取矩阵
    mat_l, names_l = extract_space_matrix("surface_L", df_l, valid_subjects, valid_stimuli, LSS_ROOT, ATLAS_SURFACE_L)
    mat_r, names_r = extract_space_matrix("surface_R", df_r, valid_subjects, valid_stimuli, LSS_ROOT, ATLAS_SURFACE_R)
    mat_v, names_v = extract_space_matrix("volume", df_v, valid_subjects, valid_stimuli, LSS_ROOT, ATLAS_VOLUME)
    ...
    logger.info(f"处理中位数版本完成! 矩阵: {matrix.shape}, 结果保存至: {OUT_DIR}")
