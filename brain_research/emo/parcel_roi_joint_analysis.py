...
# -*- coding: utf-8 -*-
"""
parcel_roi_joint_analysis.py [Atlas Config Fixed]

功能：
1. 基于 Schaefer 2018 模板 (7 Networks) 进行 ROI 级别的 ISC 联合分析。
2. 【核心修复】支持显式指定 Atlas 的 Image 路径和 Text 标签路径。
   - Surface (L/R) 共用 order_info.txt
   - Volume 使用 MNI152.txt
3. 即使 Volume 和 Surface 的脑区 ID 即使不同，也能通过各自的 txt 正确映射名称。
"""
...
def parse_schaefer_label(label_name: str) -> Tuple[str, str]:
    ...
    支持格式: '7Networks_LH_Vis_1', '7Networks_RH_Default_Par_3'
    """
    # 移除前缀以便匹配 Network 名称
    # Schaefer 的命名通常包含 LH/RH，这里统一清洗
...
def parse_schaefer_txt(txt_path: Path) -> Dict[int, str]:
    ...
    格式通常为: ID LabelName R G B A (如: 1 7Networks_LH_Vis_1 120 18 134 255)
    """
    id_to_name = {}
...
def load_atlas_labels(atlas_img_path: Path, atlas_txt_path: Path, space: str) -> Tuple[object, Dict[int, str]]:
    ...
    if space == "volume":
        ...
        # Surface: .annot (FreeSurfer) 或 .gii (Gifti Label)
        path_str = str(atlas_img_path)
        if path_str.endswith('.annot'):
            labels, ctab, names = nib.freesurfer.read_annot(path_str)
            # 注意：read_annot 自带 names，但我们要用 txt 覆盖它以保持一致性
            # annot 返回的是顶点级 labels 数组
            ...
            raise ValueError(f"不支持的 Surface Atlas 格式 (仅支持 .annot/.gii): {path_str}")
...
def extract_roi_data(
    ...
    lss_root: Path,
    preset: JointPreset,
    target_level: str,   # "network" or "parcel"
    ...
    if spec.space == "volume":
        ...
        if isinstance(atlas_obj, np.ndarray):
            ...
            # Fallback for annot tuple if not handled (should be handled in load_atlas)
            atlas_data = atlas_obj.reshape(-1)
    ...
    for uid in unique_ids_in_data:
        # 关键步骤：检查该 ID 是否在 TXT 定义的字典中
        if uid not in id_to_name: 
            # 如果 TXT 里没有这个 ID，说明是个杂项或背景，跳过
            continue
            
        label_name = id_to_name[uid]
        network_name, parcel_name = parse_schaefer_label(label_name)
    ...
    logger.info(f"  [Extract] 提取到 {len(final_masks)} 个 ROI/Networks (Map ID matches: {valid_roi_count})")
...
def run_roi_analysis(
    preset: JointPreset, 
    atlas_mapping: Dict[str, Dict[str, Path]], # 【Change】Dict of Dict
    analysis_level: str, 
    ...
    # 遍历每个 Space (Surface_L, Surface_R, Volume)
    ...
    print(f"\n>>> 统计检验: Pool={n_rois_total} ROIs (FWER corrected)")
    ...
    if not sig_df.empty:
        ...
        print(sig_df[["roi", "space", "model", "r_obs", "p_fwer"]].head(10).to_string(index=False))
...
if __name__ == "__main__":
    ...
    # 定义映射表：Key 必须对应 SimilaritySpec 中的 space
    ATLAS_MAPPING = {
        "surface_L": {
            # 图片路径 (Geometry)
    ...
    # 1. 探索大网络 (Visual, Default, etc.)
    # 这将同时加载 LH, RH, Volume 的 ROI，一起做 FWER
