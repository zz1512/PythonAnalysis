...
def main() -> None:
    ...
    p.add_argument("--aligned-csv", type=Path, required=True, help="lss_index_*_aligned.csv")
    p.add_argument("--lss-root", type=Path, required=True, help="LSS 根目录")
    p.add_argument("--task", type=str, default="", help="仅分析某个 task（EMO/SOC），为空表示不过滤")
    ...
    keys = common_stimulus_keys(df, subjects, args.min_coverage)
    if not keys:
        raise ValueError("没有满足 coverage 阈值的公共刺激。")

    lookup = build_lookup(df, args.lss_root, subjects, keys)
    ...
    print(f"公共刺激数: {len(keys)} (min_coverage={args.min_coverage})")
    print("\n--- 缺失最严重的刺激 (Top) ---")
    for k, n in missing_key_counts[: args.top_n]:
        print(f"  {k}: 缺失 {n} 人")
    print("\n--- 缺失最严重的被试 (Top) ---")
    for s, n in missing_sub_counts[: args.top_n]:
    ...
    ).to_csv(args.out_dir / "missing_stimuli_rank.csv", index=False)
    pd.DataFrame(
        {
            "subject": [s for s, _ in missing_sub_counts],
            "n_missing_stimuli": [n for _, n in missing_sub_counts],
        }
    ).to_csv(args.out_dir / "missing_subjects_rank.csv", index=False)
