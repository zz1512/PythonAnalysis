...
# -*- coding: utf-8 -*-
"""
combined_roi_joint_analysis_dev_models_perm_fdr_pos.py
...
2) 基于共同被试做 joint 分析（3 个发育模型 + permutation + FDR，仅单侧正相关）
...
3) 直接运行本脚本：
   python combined_roi_joint_analysis_dev_models_perm_fdr_pos.py

输出
-
- similarity_{name}_AgeSorted.csv（可选，save_similarity=True 时）
- subjects_common_age_sorted.csv
- joint_analysis_dev_models_perm_fdr_pos.csv（最终统计结果）
"""
...
PRESETS: Dict[str, JointPreset] = {
    "EXAMPLE_EMO_joint_all": JointPreset(
        ...
        out_dir=Path("/public/home/dingrui/fmri_analysis/zz_analysis/joint_perm_fdr_pos_out"),
...
def fdr_bh(pvals: np.ndarray) -> np.ndarray:
    ...
    n = p.size
    order = np.argsort(p)
    ranked = p[order] * float(n) / (np.arange(n, dtype=np.float64) + 1.0)
...
def run_joint_analysis(preset: JointPreset) -> Path:
    ...
    r_obs_all = np.concatenate([r_obs[m] for m in models]).astype(np.float32, copy=False)
    ...
    for _ in range(int(preset.n_perm)):
        ...
        for m in models:
            ...
            offset += n_mats
        count_raw += (r_perm_all >= r_obs_all)
    ...
    out_path = out_dir / "joint_analysis_dev_models_perm_fdr_pos.csv"
