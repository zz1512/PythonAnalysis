...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--isc-prefix", type=str, default="roi_isc_spearman_by_age")
    return p.parse_args()
...
def main() -> None:
    ...
    for d in sorted([p for p in (args.matrix_dir / "by_stimulus").iterdir() if p.is_dir()]):
        isc = np.load(d / f"{args.isc_prefix}.npy")
        subs = pd.read_csv(d / f"{args.isc_prefix}_subjects_sorted.csv")
        rois = pd.read_csv(d / f"{args.isc_prefix}_rois.csv")
