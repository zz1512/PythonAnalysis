...
def _auto_find_result_csv() -> Optional[Path]:
    ...
    valid = [p for p in cands if _is_valid_result_csv(p)]
    return _find_latest(valid)


def _is_valid_result_csv(path: Path) -> bool:
    """检查候选 CSV 是否为联合分析主结果表（排除 *_rois / *_subjects_sorted 等）。"""
    name = path.name.lower()
    if name.endswith("_rois.csv") or name.endswith("_subjects_sorted.csv"):
        return False
    try:
    ...
    required_base = {"roi", "model", "r_obs"}
    has_p = any(c in cols for c in ("p_perm_one_tailed", "p_perm", "p_raw"))
    return required_base.issubset(cols) and has_p
