...
- `behavior/joint_analysis_behavior_isc_dev_models.py`：计算行为 ISC 与三种年龄模型的关联，并做置换检验
...
- 当前行为被试 × 被试矩阵推荐从 `behavior_patterns_*` 直接构造，并使用 `--input-kind pattern --isc-method mahalanobis`
...
python behavior/calc_behavior_isc_by_age.py \
  ...
  --stimulus-dir-name by_stimulus \
  --subject-info /public/home/dingrui/fmri_analysis/data/beh/beh_indices_mri_exp_ER_TG.csv \
  --repr-prefix behavior_patterns_trial \
  --input-kind pattern \
  --isc-method mahalanobis \
  --isc-prefix behavior_pattern_isc_mahalanobis_by_age \
  --max-missing-fraction 0.2
```

主要输出：

- `behavior_pattern_isc_mahalanobis_by_age.npy`
- `behavior_pattern_isc_mahalanobis_by_age_subjects_sorted.csv`
- `behavior_pattern_isc_mahalanobis_by_age_meta.csv`
- `behavior_pattern_isc_mahalanobis_by_age_subject_filter.csv`
...
python behavior/joint_analysis_roi_isc_behavior.py \
  ...
  --behavior-repr-prefix behavior_patterns_trial \
  --behavior-isc-method mahalanobis \
  --behavior-isc-prefix behavior_pattern_isc_mahalanobis_by_age \
  --assoc-method spearman \
  --tail two_sided \
...
python behavior/joint_analysis_roi_isc_behavior.py \
  ...
  --brain-isc-method mahalanobis \
  --behavior-repr-prefix behavior_patterns_emotion4 \
  --behavior-isc-method mahalanobis \
  --behavior-isc-prefix behavior_pattern_isc_mahalanobis_by_age \
  --assoc-method spearman \
  --tail two_sided \
...
#### 5.4 行为 ISC 与三种年龄模型相关

trial：
```bash
python behavior/joint_analysis_behavior_isc_dev_models.py \
  --matrix-dir /public/home/dingrui/fmri_analysis/zz_analysis/roi_results_final \
  --stimulus-dir-name by_stimulus \
  --behavior-isc-method mahalanobis \
  --behavior-isc-prefix behavior_pattern_isc_mahalanobis_by_age \
...
主要输出：
- `behavior_isc_dev_models_perm_fwer.csv`
- `behavior_isc_dev_models_perm_fwer_subjects_sorted.csv`
- `behavior_isc_dev_models_perm_fwer_by_stimulus_summary.csv`
- `behavior_isc_dev_models_perm_fwer_by_emotion_summary.csv`

其中 `behavior_isc_dev_models_perm_fwer.csv` 每个条件输出 3 行，对应 `M_nn`、`M_conv`、`M_div`。重点查看：
- `model`
- `r_obs`
- `p_perm`
- `p_fwer_global`
...
如果只需要重算行为被试 × 被试矩阵并检验行为与三种年龄模型的相关，可先运行：

```bash
python run_pipeline.py --config behavior/config_behavior_trial.json --steps beh_isc_trial
python behavior/joint_analysis_behavior_isc_dev_models.py \
  --matrix-dir /public/home/dingrui/fmri_analysis/zz_analysis/roi_results_final \
  --stimulus-dir-name by_stimulus \
  --behavior-isc-method mahalanobis \
...
```text
Behavior_ISC = β1 * Brain_ISC + β2 * Age_Model + β3 * (Brain_ISC * Age_Model) + ε
```
...
- 不再输出缺乏统计解释意义的参数法 `p_brain / p_age / p_interaction`
...
- `roi_isc_behavior_age_regression_by_stimulus_summary.csv`
- `roi_isc_behavior_age_regression_by_emotion_summary.csv`
- `roi_isc_behavior_age_regression_<model>_permmax_checkpoint.npz`：每个年龄模型的 maxT 置换进度文件，用于断点续跑和最终 FWER 回填
...
- `roi_isc_behavior_age_regression_by_stimulus_summary.csv` / `roi_isc_behavior_age_regression_by_emotion_summary.csv`：各 `stimulus_type × model` 下显著交互项 ROI 数量汇总
- 年龄调制脚本会在每个 `model × ROI` 完成后立即更新 `roi_isc_behavior_age_regression.csv`；如果中断后用同一命令重跑，会读取已有 CSV 和对应 `permmax_checkpoint.npz`，跳过已完成 ROI，并在每个模型全部 ROI 完成后回填准确的 model-wise FWER。
- CSV 中的 `analysis_signature` 用来校验当前参数、被试、ROI 和矩阵维度是否与断点结果一致；不一致会自动重算。`model_complete` 为 `True` 时，该年龄模型的 FWER 已完成回填。
