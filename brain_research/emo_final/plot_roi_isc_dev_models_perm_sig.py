...
def resolve_sig_method(sig_method: Optional[str], fdr_mode: str) -> str:
    ...
    # Backward-compatible switch: --fdr-mode maps to a concrete column in Step3 outputs.
...
def collect_results(
    ...
    for d in sorted([p for p in by_stim.iterdir() if p.is_dir()]):
        ...
        sm = str(sig_method)
        if sm == "fwer":
            # FWER: model-wise maxT correction from Step3.
            ...
            # FDR (BH) within each model separately.
            ...
            # FDR (BH) across all tests in this stimulus_type.
