...
"""
emo_new：按 3 个 stimulus_type 条件绘制 ROI ISC 显著热图（FDR-BH 版本）。

输入默认来自：
  <roi_results>/by_stimulus/<stimulus_type>/roi_isc_dev_models_perm_fwer.csv

流程：
1) 读取原始置换 p（优先 p_perm_one_tailed）
2) 按指定范围计算 BH-FDR（model_wise 或 global）
3) 对 3 个条件并排绘图
"""
...
def _prepare_sig_df(result_csv: Path, alpha: float, positive_only: bool, fdr_mode: str) -> pd.DataFrame:
    ...
    if p_raw_col is None:
        raise ValueError(f"{result_csv} 缺少原始 p 列")

    req = {"roi", "model", "r_obs", p_raw_col}
    miss = req - set(df.columns)
    if miss:
        raise ValueError(f"{result_csv} 缺少列: {sorted(miss)}")

    work = _calc_fdr(df, p_raw_col=p_raw_col, mode=fdr_mode)
...
def plot_three_conditions(
    ...
    for stim in stimuli:
        ...
        sig_by_cond[stim] = _prepare_sig_df(cond2csv[stim], alpha=float(alpha), positive_only=bool(positive_only), fdr_mode=fdr_mode)

    union = []
    for stim in stimuli:
        sub = sig_by_cond[stim]
        if not sub.empty:
            union.append(sub[["roi", "p_fdr_bh"]].groupby("roi", as_index=False)["p_fdr_bh"].min())
    if not union:
        raise ValueError("3 个条件下都没有 FDR 显著 ROI，无法绘图")

    merged = pd.concat(union, axis=0, ignore_index=True)
    roi_order = merged.groupby("roi", as_index=False)["p_fdr_bh"].min().sort_values("p_fdr_bh")["roi"].astype(str).tolist()
    ...
    mats_q: Dict[str, pd.DataFrame] = {}
    vmax = 0.1
    for stim in stimuli:
        sub = sig_by_cond[stim]
        if sub.empty:
            mat_r = pd.DataFrame(np.nan, index=roi_order, columns=models)
            mat_q = pd.DataFrame(np.nan, index=roi_order, columns=models)
        else:
    ...
    for i, stim in enumerate(stimuli):
        ...
        for r in range(annot.shape[0]):
            for c in range(annot.shape[1]):
                rv = mat_r.iat[r, c]
                qv = mat_q.iat[r, c]
                annot.iat[r, c] = f"{rv:.2f}\n(q={qv:.3g})" if np.isfinite(rv) and np.isfinite(qv) else ""
    ...
    title = f"emo_new Significant ROI ISC Effects (BH-FDR; q≤{alpha:g}; {fdr_mode})"
    if positive_only:
        title += "\n(positive r only)"
    ...
    fig_out = out_dir / f"emo_new_roi_isc_dev_models_sig_heatmap_3conds__{stim_tag}__fdr_{fdr_mode}__a{alpha:g}.{fmt}"
    ...
    sig_table = sig_table.sort_values(["stimulus_type", "model", "p_fdr_bh", "r_obs"], ascending=[True, True, True, False])
    csv_out = out_dir / f"emo_new_roi_isc_dev_models_sig_table_3conds__{stim_tag}__fdr_{fdr_mode}__a{alpha:g}.csv"
...
def plot_each_condition(
    ...
    for stim in stimuli:
        if stim not in cond2csv:
            raise FileNotFoundError(f"条件 {stim} 缺少结果文件")
        sub = _prepare_sig_df(cond2csv[stim], alpha=float(alpha), positive_only=bool(positive_only), fdr_mode=fdr_mode)
        if sub.empty:
            continue

        roi_order = (
            sub[["roi", "p_fdr_bh"]]
            .groupby("roi", as_index=False)["p_fdr_bh"]
            .min()
            .sort_values("p_fdr_bh")["roi"]
            .astype(str)
            .tolist()
        )
        ylabels = [_build_roi_display(r, map_l, map_r) for r in roi_order]
        ...
        fig_out = out_dir / f"emo_new_roi_isc_dev_models_sig_heatmap_single__{stim}__fdr_{fdr_mode}__a{alpha:g}.{fmt}"
        fig.savefig(fig_out, dpi=dpi, bbox_inches="tight")
        plt.close(fig)
        outs.append(fig_out)

        sub_out = sub.copy().sort_values(["model", "p_fdr_bh", "r_obs"], ascending=[True, True, False])
        sub_out.insert(0, "stimulus_type", stim)
        csv_out = out_dir / f"emo_new_roi_isc_dev_models_sig_table_single__{stim}__fdr_{fdr_mode}__a{alpha:g}.csv"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(description="emo_new: 3 条件 FDR 显著 ROI 热图")
