...
def plot_hypothesis_violin(subject_hypo_df: pd.DataFrame):
    ...
    for roi_name in sorted(subject_hypo_df["roi"].unique()):
        ...
        fig, axes = plt.subplots(1, len(tests), figsize=(6.2 * len(tests), 5.2), sharey=True)
        if len(tests) == 1:
            axes = [axes]
        ...
        for ax, test_name in zip(axes, tests):
            ...
            for _, row in sub_df.iterrows():
                ax.plot(positions, [row["left_r"], row["right_r"]], color="gray", alpha=0.25, linewidth=0.9, zorder=1)
                ax.scatter(positions, [row["left_r"], row["right_r"]], color=["#d62728", "#1f77b4"], s=16, alpha=0.85, zorder=2)
...
def run_fertility_score_modulation(subject_hypo_df: pd.DataFrame, fdr_mode: str = "by_test") -> pd.DataFrame:
    ...
    if len(out_df) > 0:
        if out_df["lin_p_two_sided"] >0:
            out_df["p_one_sided"] = out_df["lin_p_two_sided"] / 2.0
        else:
            out_df["p_one_sided"] = 1 - out_df["lin_p_two_sided"] / 2.0
...
def main():
    ...
    plot_hypothesis_violin(subject_hypo_df)
