...
# -*- coding: utf-8 -*-
"""
run_rsa_exp2_threat_rois_v4_6.py
...
TRIAL_HEATMAP_MASK_DIAGONAL = False  # 老师要求：trial 图对角线保留为 1
TRIAL_HEATMAP_MASK_SAME_RUN = False  # trial 图作为展示图，不再 mask 同 run
SAVE_TARGET_PAIR_PLOTS = False  # 默认不再输出 target pair 分页图
HEATMAP_SHOW_TEXT = True
PUBLICATION_FIG_DIR = OUTPUT_DIR / "figures_publication"
...
logger = logging.getLogger("RSA_EXP2_V4_6")
...
def item_sort_value(item_key: str):
    """
    返回稳定可比较的排序键，避免 int/str 混排引发排序错误。
    """
...
def get_condition_sort_order(labels: List[str], item_keys: List[str] | None = None) -> np.ndarray:
    ...
    使用 Python sorted，避免 np.lexsort 在 object/tuple 排序时的 shape 问题。
    """
    rank_map = {c: i for i, c in enumerate(COND_ORDER)}

    if item_keys is None:
        order = sorted(range(len(labels)), key=lambda i: (rank_map.get(labels[i], 999), i))
...
def compute_trial_heatmap_range(display_mat: np.ndarray) -> Tuple[float, float]:
    ...
    # 对角线可能为 1；范围估计时不让其主导色条
    vals_off = vals[np.abs(vals) < 0.999]
    vals_use = vals_off if vals_off.size > 10 else vals
    if TRIAL_HEATMAP_AUTO_RANGE:
        lo = float(np.quantile(vals_use, TRIAL_HEATMAP_QLOW))
        hi = float(np.quantile(vals_use, TRIAL_HEATMAP_QHIGH))
        lo = max(lo, TRIAL_HEATMAP_VMIN)
        hi = min(hi, TRIAL_HEATMAP_VMAX)
...
def plot_group_sorted_trial_heatmaps(trial_records: List[Dict]):
    ...
    组平均排序 trial 热图（展示图）：
    - 默认保留对角线为 1
    - 默认不 mask 同 run pairs
    - 组平均时对每个 cell 先 Fisher z 再平均，再反变换回 r
    """
    if not trial_records:
        return

    fig_dir = PUBLICATION_FIG_DIR / "trial_heatmaps"
    fig_dir.mkdir(parents=True, exist_ok=True)
    plotdata_dir = PLOT_DATA_DIR / "trial_heatmaps"
    ...
    for roi_name in roi_names:
        ...
        trial_ids = [f"{lab}|{item}" for lab, item in zip(base_labels, base_items)]
        pd.DataFrame(mean_mat, index=trial_ids, columns=trial_ids).to_csv(
            plotdata_dir / f"trial_heatmap_group_{roi_name}.csv", encoding="utf-8-sig"
        )
        pd.DataFrame({
            "trial_id": trial_ids,
            "label": base_labels,
            "item_key": base_items,
            "run": base_runs,
        }).to_csv(plotdata_dir / f"trial_heatmap_group_{roi_name}_order.csv", index=False, encoding="utf-8-sig")
...
def plot_hypothesis_violin(subject_hypo_df: pd.DataFrame, hypo_df: pd.DataFrame):
    ...
    - 图上直接显示 q/p/t
    """
    fig_dir = PUBLICATION_FIG_DIR / "violin"
    fig_dir.mkdir(parents=True, exist_ok=True)
...
def plot_fertility_score_scatter(subject_hypo_df: pd.DataFrame):
    ...
    fig_dir = PUBLICATION_FIG_DIR / "fertility_score"
    fig_dir.mkdir(parents=True, exist_ok=True)
...
def plot_group_heatmaps(matrix_df: pd.DataFrame):
    fig_dir = PUBLICATION_FIG_DIR / "heatmaps_6x6"
    fig_dir.mkdir(parents=True, exist_ok=True)
...
def plot_model_fit_bars(model_subj_df: pd.DataFrame, model_group_df: pd.DataFrame):
    ...
    - 标注 q/p
    """
    fig_dir = PUBLICATION_FIG_DIR / "model_fit"
    fig_dir.mkdir(parents=True, exist_ok=True)
...
def save_plot_ready_tables(
    matrix_all: pd.DataFrame,
    subject_hypo_df: pd.DataFrame,
    hypo_df: pd.DataFrame,
    spec_df: pd.DataFrame,
    model_subj_df: pd.DataFrame,
    model_group_df: pd.DataFrame,
    fert_df: pd.DataFrame | None = None,
    fert_group_subject_df: pd.DataFrame | None = None,
    fert_group_summary_df: pd.DataFrame | None = None,
):
    """
    保存可直接重绘的 plot-ready tables，后续重新绘图不需要重新分析。
    """
    tables_dir = OUTPUT_DIR / "tables"
    tables_dir.mkdir(parents=True, exist_ok=True)
    plot_dir = PLOT_DATA_DIR
    plot_dir.mkdir(parents=True, exist_ok=True)

    matrix_all.to_csv(tables_dir / "rsa_category_similarity_matrix_rows.csv", index=False, encoding="utf-8-sig")
    subject_hypo_df.to_csv(tables_dir / "rsa_subjectwise_hypothesis_lr.csv", index=False, encoding="utf-8-sig")
    hypo_df.to_csv(tables_dir / "rsa_hypothesis_tests.csv", index=False, encoding="utf-8-sig")
    spec_df.to_csv(tables_dir / "rsa_specificity_tests.csv", index=False, encoding="utf-8-sig")
    model_subj_df.to_csv(tables_dir / "rsa_model_rdm_subjectwise.csv", index=False, encoding="utf-8-sig")
    model_group_df.to_csv(tables_dir / "rsa_model_rdm_group_tests.csv", index=False, encoding="utf-8-sig")
    if fert_df is not None:
        fert_df.to_csv(tables_dir / "rsa_fertility_score_modulation.csv", index=False, encoding="utf-8-sig")
    if fert_group_subject_df is not None:
        fert_group_subject_df.to_csv(tables_dir / "rsa_fertility_score_grouped_subjectwise.csv", index=False, encoding="utf-8-sig")
    if fert_group_summary_df is not None:
        fert_group_summary_df.to_csv(tables_dir / "rsa_fertility_score_grouped_summary.csv", index=False, encoding="utf-8-sig")

    hm_dir = plot_dir / "heatmaps_6x6"
    hm_dir.mkdir(parents=True, exist_ok=True)
    for roi_name in sorted(matrix_all["roi"].unique()):
        roi_df = matrix_all[matrix_all["roi"] == roi_name].copy()
        mean_mat = roi_df.groupby("cond_row")[COND_ORDER].mean().loc[COND_ORDER, COND_ORDER]
        mean_mat.to_csv(hm_dir / f"heatmap6x6_{roi_name}.csv", encoding="utf-8-sig")

    violin_df = subject_hypo_df.merge(
        hypo_df[["roi", "test", "t", "p_one_sided", "q_fdr_one_sided", "mean_left_r", "mean_right_r"]],
        on=["roi", "test"],
        how="left"
    )
    violin_dir = plot_dir / "violin"
    violin_dir.mkdir(parents=True, exist_ok=True)
    violin_df.to_csv(violin_dir / "violin_plotdata.csv", index=False, encoding="utf-8-sig")

    model_dir = plot_dir / "model_fit"
    model_dir.mkdir(parents=True, exist_ok=True)
    model_subj_df.to_csv(model_dir / "model_fit_subjectwise.csv", index=False, encoding="utf-8-sig")
    model_group_df.to_csv(model_dir / "model_fit_group.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    logger.info("=== 开始实验2 ROI-RSA v4.6 论文图优化版分析 ===")
    ...
    logger.info(f"PUBLICATION_FIG_DIR = {PUBLICATION_FIG_DIR}")
    logger.info(f"PLOT_DATA_DIR = {PLOT_DATA_DIR}")
    ...
    fert_df = None
    fert_group_subject_df = None
    fert_group_summary_df = None
    ...
    save_plot_ready_tables(
        matrix_all, subject_hypo_df, hypo_df, spec_df, model_subj_df, model_group_df,
        fert_df=fert_df,
        fert_group_subject_df=fert_group_subject_df,
        fert_group_summary_df=fert_group_summary_df,
    )

    logger.info("=== RSA 论文图优化版分析完成 ===")
...
def plot_fertility_group_violin(subject_hypo_df: pd.DataFrame):
    ...
    fig_dir = PUBLICATION_FIG_DIR / "fertility_groups"
    fig_dir.mkdir(parents=True, exist_ok=True)
    palette = {
        "低生育意愿": "#4c72b0",
        "中生育意愿": "#55a868",
