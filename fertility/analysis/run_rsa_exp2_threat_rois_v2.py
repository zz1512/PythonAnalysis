...
# -*- coding: utf-8 -*-
"""
run_rsa_exp2_threat_rois_v2.py

实验2（威胁情境任务）ROI-RSA 增强版
-------------------------------------------------
在现有脚本基础上新增：
1. 特异性检验（specificity tests）
2. 模型 RDM 检验（model-RSA）
3. 中文字体自动配置，修复图片中文乱码
4. GPU 可选加速（优先 CuPy，其次 PyTorch；主要加速 trial×trial 相似性矩阵计算）
...
def build_category_similarity_tables(sim_mat: np.ndarray, labels: List[str], subject: str, roi_name: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    long_rows = []
    matrix_rows = []
...
def process_subject(sub: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for roi_name, roi_mask_path in ROI_MASKS.items():
        ...
        long_df, matrix_df = build_category_similarity_tables(sim_mat, labels, sub, roi_name)
...
def main():
    ...
    logger.info("=== 开始实验2 ROI-RSA 增强版分析 ===")
