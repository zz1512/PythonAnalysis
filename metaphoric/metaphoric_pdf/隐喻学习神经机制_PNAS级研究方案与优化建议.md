...
隐喻联结学习不仅改变了脑区的激活强度，更深层地重组了多变量编码模式和表征几何结构；主分析聚焦 yy vs kj 的学习差异，同时用 baseline 作为控制验证（可选）来检验观察到的变化是否可能由重复暴露效应解释。这种重组跨层次地预测了行为学习效果。
...
- 核心对比：主分析为 **2（条件：yy vs kj）× 2（时间：pre vs post）**；baseline用于控制验证（可选）
- **注意**：baseline条件**不参与学习（Run3-4）和回忆（Run7）**，仅出现在前后测（Run1-2, 5-6）中，可用于评估重测/重复暴露效应
...
> 以下问题来自实验流程文档与当前方案/代码的系统交叉审查。结合我们刚刚的讨论：**问题A是PNAS审稿很可能追问的关键控制点（推荐做控制验证，但不影响yy-kj主分析成立）**；问题B是提升证据链完整性的可选增强项。

## 问题A（关键控制，推荐）：baseline基线条件未纳入控制验证

### 问题描述

实验设计是 **3条件**（隐喻联结yy / 空间联结kj / 不联结baseline），但当前方案和几乎全部代码（15/19个文件）都只假设了2个条件（yy/kj），baseline没有进入前后测表征分析链路。

需要澄清的是：\n+1. **如果你的核心问题是“隐喻学习是否比空间学习引起更大的变化”**，那么主分析使用 `Δyy - Δkj`（difference-in-differences）在“重测效应对两条件近似等量”的前提下，确实会抵消掉这部分重复暴露效应。\n+2. baseline的价值主要在于回答另一个审稿人常问的问题：**“观察到的变化是不是学习本身导致的，而不是普遍的重测/熟悉化效应？”** 这需要验证 `Δbaseline` 是否接近0，以及 `Δyy`、`Δkj` 是否显著大于 `Δbaseline`。\n+\n+因此，baseline更适合作为**控制验证/稳健性检查**，而不是必须进入所有主分析。

### 影响范围

| 文件 | 当前状态 | 如加入baseline控制验证需要修正 |
|------|---------|---------|
...
**核心原则**：baseline在不同分析阶段的角色不同，且更适合做“控制验证”
...
2. **控制验证（推荐）**：增加 baseline 的前后测变化作为"无学习"对照
...
4. 统计方案建议分层：\n+   - **主分析**：2（yy/kj）× 2（pre/post）重复测量ANOVA或LMM\n+   - **控制验证**：加入baseline后做 3（yy/kj/baseline）× 2（pre/post）重复测量ANOVA，或做两条关键不等式检验（`Δyy > Δbaseline`、`Δkj > Δbaseline`）并做FDR校正
...
## 建议修改的现有脚本（对应问题A-E，按“主分析必需/控制验证/增强”分层）

| 优先级 | 脚本 | 修改内容 | 对应问题 |
|--------|------|---------|---------|
| **P0（主分析必需）** | `representation_analysis/stack_patterns.py` | 明确假词排除逻辑；保证pre/post条件标签一致（yy/kj） | E |
| **P1（控制验证，推荐）** | `representation_analysis/stack_patterns.py` | `CONDITION_MAP` 添加baseline（仅用于控制验证） | A |
| **P1（控制验证，推荐）** | `common/final_utils.py` | 增加baseline控制的对比辅助函数（如`delta_vs_baseline()`）；保留现有`difference_in_differences()`用于yy-kj主分析 | A |
| **P1（控制验证，推荐）** | `representation_analysis/gps_analysis.py` | 可选加入baseline输出与`Δbaseline`检验 | A |
| **P1（控制验证，推荐）** | `representation_analysis/rd_analysis.py` | 可选加入baseline输出与`Δbaseline`检验 | A |
| **P1（控制验证，推荐）** | `rsa_analysis/cross_roi_rsa.py` | 可选加入baseline输出与`Δbaseline`检验 | A |
| **P1（控制验证，推荐）** | `rsa_analysis/model_rdm_comparison.py` | 可选加入baseline作为模型/协变量（如果要做3条件模型） | A |
| **P2（增强，非必需）** | `glm_analysis/glm_config.py` | 新增前后测Run配置和3条件对比 | B |
| **P2（增强，非必需）** | `glm_analysis/main_analysis.py` | 支持前后测GLM + 交互效应分析 | B |
...
| **P1（推荐）** | `behavior_analysis/prepost_behavior.py` | 前后测真假词判断的3条件×2时间行为分析（可用于解释表征变化是否伴随行为变化） | D |
...
### 4D. 模型RDM比较（Model-RSA 核心四模型：M1/M2/M3/M7）

- **目的**：用理论驱动的模型 RDM 解释"学习前预存语义 → 学习后配对结构与记忆强度重塑"的核心故事。
- **落地的四个核心模型**（实现于 `rsa_analysis/model_rdm_comparison.py`）：
  - M1_condition：三水平条件 RDM（yy/kj/baseline，同条件=0，跨条件=1）
  - M2_pair：配对身份 RDM（同 pair_id=0，其余=1）——学习配对对齐的直接证据
  - M3_embedding：BERT 预训练语义 cosine distance —— 预存语义基线
  - M7_memory：per-subject 回忆强度差 `|recall_i − recall_j|` —— 行为-表征桥梁
- **统计框架**：
  - 单模型 Spearman ρ（pre/post 独立）
  - `--partial-correlation`：多模型偏相关（残差化法），区分独立解释方差
  - `model_collinearity.tsv`：|ρ|>0.7 标 high，防止偏相关被共线性污染
  - `rsa_analysis/delta_rho_lmm.py`：Δρ = post − pre，拟合 `delta_rho ~ C(model) + (1|subject) + (1|roi)`
- **预计算依赖**：
  - `rsa_analysis/build_stimulus_embeddings.py`（一次性产出词向量 TSV）
  - `rsa_analysis/build_memory_strength_table.py`（一次性产出 per-subject 记忆强度表）
- ✅ **已实现**：`metaphoric/final_version/rsa_analysis/model_rdm_comparison.py`（含四大模型 + 偏相关 + 共线性 + 三层输出目录自动 tag）
- ✅ **已实现**：`metaphoric/final_version/rsa_analysis/delta_rho_lmm.py`
- **参考方法**：Kriegeskorte et al. (2008) RSA toolbox；Bhattasali et al. 2018（BERT vs 神经）；Xue et al. 2010（记忆成功 ↔ 神经模式稳定性）；Cardillo et al. 2012（隐喻 novel→familiar tuning）
...
- [x] N1: stack_patterns.py — 模式堆叠（已在 `metaphoric/final_version/representation_analysis/stack_patterns.py` 实现）
- [x] N2: rd_analysis.py — 表征维度分析（已在 `metaphoric/final_version/representation_analysis/rd_analysis.py` 实现）
- [x] N3: gps_analysis.py — GPS分析（已在 `metaphoric/final_version/representation_analysis/gps_analysis.py` 实现）
- [x] N4: cross_roi_rsa.py — 跨脑区RSA（已在 `metaphoric/final_version/rsa_analysis/cross_roi_rsa.py` 实现）
- [x] N5: rsa_lmm.py — RSA LMM统计（已在 `metaphoric/final_version/rsa_analysis/rsa_lmm.py` 实现）
- [x] N6: brain_behavior_correlation.py — 脑-行为关联（已在 `metaphoric/final_version/brain_behavior/brain_behavior_correlation.py` 实现）

### Phase 3：新建扩展脚本（中优先级 N7-N12）

- [x] N7: rd_searchlight.py — 全脑RD map（已在 `metaphoric/final_version/representation_analysis/rd_searchlight.py` 实现）
- [x] N8: seed_connectivity.py — 种子连接性（已在 `metaphoric/final_version/representation_analysis/seed_connectivity.py` 实现）
- [x] N9: rd_robustness.py — RD稳健性（已在 `metaphoric/final_version/representation_analysis/rd_robustness.py` 实现）
- [x] N10: mvpa_pre_post_comparison.py — 前后测MVPA（已在 `metaphoric/final_version/fmri_mvpa/fmri_mvpa_roi/mvpa_pre_post_comparison.py` 实现）
- [x] N11: cross_phase_decoding.py — 跨阶段解码（已在 `metaphoric/final_version/fmri_mvpa/fmri_mvpa_roi/cross_phase_decoding.py` 实现）
- [x] N12: model_rdm_comparison.py — 模型RDM比较（已在 `metaphoric/final_version/rsa_analysis/model_rdm_comparison.py` 实现）

### Phase 4：新建补充脚本（文献Gap补充 N13-N17）

- [x] N13: parametric_modulation.py — 熟悉度参数调制GLM（已在 `metaphoric/final_version/glm_analysis/parametric_modulation.py` 实现）
- [x] N14: lateralization_analysis.py — 偏侧化指数分析（已在 `metaphoric/final_version/lateralization/lateralization_analysis.py` 实现）
- [x] N15: learning_dynamics.py — 学习阶段时间窗动态（已在 `metaphoric/final_version/temporal_dynamics/learning_dynamics.py` 实现）
- [x] N16: mediation_analysis.py — 中介分析（已在 `metaphoric/final_version/brain_behavior/mediation_analysis.py` 实现）
- [x] N17: effective_connectivity.py — 有效连接（已在 `metaphoric/final_version/connectivity_analysis/effective_connectivity.py` 实现）

### Phase 5：新建统计工具脚本（N18-N21）

- [x] N18: gPPI_analysis.py — gPPI功能连接（已在 `metaphoric/final_version/connectivity_analysis/gPPI_analysis.py` 实现）
- [x] N19: behavior_lmm.py — 行为LMM（已在 `metaphoric/final_version/behavior_analysis/behavior_lmm.py` 实现）
- [x] N20: bayesian_stats.py — 贝叶斯因子（已在 `metaphoric/final_version/utils/bayesian_stats.py` 实现）
- [x] N21: bootstrap_ci.py — Bootstrap CI（已在 `metaphoric/final_version/utils/bootstrap_ci.py` 实现）
