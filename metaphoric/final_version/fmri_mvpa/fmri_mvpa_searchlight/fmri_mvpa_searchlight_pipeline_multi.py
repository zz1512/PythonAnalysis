...
class MVPAConfig:
    ...
    def __init__(self, overrides=None):
        ...
        self.mask_dir = Path(r"E:/python_metaphor/glm_analysis_fwe_final/2nd_level_CLUSTER_FWE/Metaphor_gt_Spatial")  # ?????? group_mask ????
        self.results_dir = Path(r"E:/python_metaphor/mvpa_searchlight_results")
        ...
        if not (self.mask_dir / self.process_mask).exists() and (self.mask_dir / 'group_mask.nii.gz').exists():
            self.process_mask = 'group_mask.nii.gz'
...
def prepare_classification_data(trial_info, beta_images, cond1, cond2, config):
    ...
    # 筛选指定条件的trial
    cond1_trials = trial_info[get_condition_mask(trial_info, condition_col, cond1)]
    cond2_trials = trial_info[get_condition_mask(trial_info, condition_col, cond2)]
    ...
    if getattr(config, 'balance_trials', False):
        ...
        if ('leave-one' in cv_strategy and 'run' in cv_strategy) and run_column is not None:
            ...
            for run_id, run_trials in trial_info.groupby(run_column, sort=False):
                run_cond1 = run_trials[get_condition_mask(run_trials, condition_col, cond1)]
                run_cond2 = run_trials[get_condition_mask(run_trials, condition_col, cond2)]
    ...
    trial_summary['n_trials_cond1_used'] = int(get_condition_mask(selected_trials, condition_col, cond1).sum())
    trial_summary['n_trials_cond2_used'] = int(get_condition_mask(selected_trials, condition_col, cond2).sum())

    # 获取对应的beta图像
    selected_indices = selected_trials['combined_trial_index'].astype(int).values
...
def process_searchlight_results_batch(beta_images, labels, process_mask_path, config,
    ...
    if subject_results and trial_details is not None:
        ...
        if condition_col in trial_details.columns:
            subject_results['n_trials_cond1'] = int(get_condition_mask(trial_details, condition_col, cond1).sum())
            subject_results['n_trials_cond2'] = int(get_condition_mask(trial_details, condition_col, cond2).sum())
