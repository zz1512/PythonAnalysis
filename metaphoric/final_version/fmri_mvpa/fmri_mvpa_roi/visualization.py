...
def create_enhanced_visualizations(group_df, stats_df, config):
    ...
    # 动态创建标签和颜色，确保标签数量与数据数量匹配
    labels = []
    colors_pie = []

    if True in sig_counts:
        labels.append('显著')
        colors_pie.append('#2E8B57')
    if False in sig_counts:
        labels.append('不显著')
        colors_pie.append('#CD5C5C')

    if labels:  # 确保有数据时才创建饼图
...
def create_individual_roi_plots(group_df, stats_df, config):
    ...
    # 如果没有ROI数据，直接返回
    if n_rois == 0:
        log("警告：没有ROI数据，跳过创建单个ROI图表", config)
        return None
    ...
    for idx, roi in enumerate(rois):
        ...
        # 获取该ROI的数据
        roi_data = group_df[group_df['roi'] == roi]['mean_accuracy']
        roi_stats_row = stats_df[stats_df['roi'] == roi]

        if len(roi_stats_row) == 0:
            continue

        roi_stats = roi_stats_row.iloc[0]
