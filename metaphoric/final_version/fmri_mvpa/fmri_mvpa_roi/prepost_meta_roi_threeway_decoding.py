"""Pre/post three-way condition decoding in planned meta ROIs.

Conditions are YY, KJ, and JX.  In the stacked pattern files JX is stored as
`baseline`, so this script maps baseline -> jx for reporting.  The primary
test is run-held-out decoding within each phase:
...
CONDITION_TO_LABEL = {"jx": 0, "kj": 1, "yy": 2}
STORED_TO_CONDITION = {"baseline": "jx", "kj": "kj", "yy": "yy"}
...
def _one_sample_accuracy(values: pd.Series, chance: float = 1 / 3) -> dict[str, float]:
...
def _load_condition(subject_dir: Path, phase: str, stored_condition: str) -> tuple[pd.DataFrame, np.ndarray]:
    image_path = subject_dir / f"{phase}_{stored_condition}.nii.gz"
    metadata_path = subject_dir / f"{phase}_{stored_condition}_metadata.tsv"
    if not image_path.exists() or not metadata_path.exists():
        raise FileNotFoundError(f"Missing {phase}_{stored_condition} for {subject_dir.name}")
    ...
    condition = STORED_TO_CONDITION[stored_condition]
    metadata["subject"] = metadata.get("subject", subject_dir.name).astype(str)
    metadata["phase_label"] = phase
    metadata["stored_condition"] = stored_condition
    metadata["condition3"] = condition
...
def _make_dataset(subject_dir: Path, mask: np.ndarray, mask_path: Path) -> pd.DataFrame:
    ...
    for phase in ["pre", "post"]:
        for stored_condition in ["baseline", "kj", "yy"]:
            metadata, data = _load_condition(subject_dir, phase, stored_condition)
            if data.shape[:3] != mask.shape:
                raise ValueError(f"Image/mask shape mismatch: {subject_dir / f'{phase}_{stored_condition}.nii.gz'} vs {mask_path}")
...
def run_heldout_phase_decoding(data: pd.DataFrame, *, subject: str, roi_set: str, roi: str, phase: str) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    for train_run, test_run in [(runs[0], runs[1]), (runs[1], runs[0])]:
        ...
        if train["label"].nunique() < 3 or test["label"].nunique() < 3:
            continue
        clf = _classifier()
        ...
        direction_rows.append(
            {
                ...
                "roi_set": roi_set,
                "roi": roi,
                "analysis_type": f"{phase}_run_heldout_threeway",
                "phase": phase,
    ...
    return [
        {
            ...
            "roi_set": roi_set,
            "roi": roi,
            "analysis_type": f"cross_phase_{train_phase}_to_{test_phase}_threeway",
...
def summarize_decoding(subject_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, subset in subject_metrics.groupby(["roi_set", "roi", "analysis_type"], sort=False):
        ...
        summary = _one_sample_accuracy(subset["balanced_accuracy"], chance=1 / 3)
        rows.append({"roi_set": roi_set, "roi": roi, "analysis_type": analysis_type, "metric": "balanced_accuracy", "chance": 1 / 3, **summary})
    ...
    primary = out["analysis_type"].isin(["pre_run_heldout_threeway", "post_run_heldout_threeway"])
...
def summarize_post_minus_pre(subject_metrics: pd.DataFrame) -> pd.DataFrame:
    phase_rows = subject_metrics[subject_metrics["analysis_type"].isin(["pre_run_heldout_threeway", "post_run_heldout_threeway"])].copy()
    rows: list[dict[str, object]] = []
    for keys, subset in phase_rows.groupby(["roi_set", "roi"], sort=False):
        ...
        if {"pre_run_heldout_threeway", "post_run_heldout_threeway"} - set(pivot.columns):
            continue
        summary = _paired_delta(pivot["post_run_heldout_threeway"], pivot["pre_run_heldout_threeway"])
        rows.append({"roi_set": roi_set, "roi": roi, "analysis_type": "post_minus_pre_threeway", **summary})
    out = pd.DataFrame(rows)
    if out.empty:
        return out
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Pre/post YY/KJ/JX three-way decoding in meta ROIs.")
    ...
    output_dir = args.paper_output_root / "qc" / "prepost_threeway_mvpa_decoding"
        ...
        for (roi_set, roi_name), mask_path in selected_masks.items():
            try:
                ...
                for phase in ["pre", "post"]:
                    ...
                    for condition in ["jx", "kj", "yy"]:
                        qc[f"n_{phase}_{condition}"] = int(phase_data["condition3"].eq(condition).sum())
    ...
    write_table(subject_metrics, output_dir / "prepost_threeway_mvpa_subject_metrics.tsv")
    write_table(direction_metrics, output_dir / "prepost_threeway_mvpa_direction_metrics.tsv")
    write_table(qc, output_dir / "prepost_threeway_mvpa_qc.tsv")
    write_table(failures, output_dir / "prepost_threeway_mvpa_failures.tsv")
    write_table(group, output_dir / "prepost_threeway_mvpa_group_fdr.tsv")
    write_table(delta, output_dir / "prepost_threeway_mvpa_post_minus_pre_fdr.tsv")
    write_table(group, tables_main / "table_prepost_threeway_mvpa_decoding.tsv")
    write_table(delta, tables_main / "table_prepost_threeway_mvpa_post_minus_pre.tsv")
    write_table(subject_metrics, tables_si / "table_prepost_threeway_mvpa_subject_metrics.tsv")
