...
结果: YY 和 KJ 在反应时上没有显著差异
    ...
    Cohen's dz = -0.189
    Shapiro 正态性检验（差值）：W = 0.9658, p = 0.495581 (正态)

    === 独立样本 Welch t检验（补充分析，试次级数据） ===
    YY 试次数 = 1890, KJ 试次数 = 1890
    YY 试次均值 = 1028.2862 ms, KJ 试次均值 = 1039.8540 ms
    t ≈ -0.8203, p = 0.412092, Hedges' g = -0.027

反应时行为结果总结：
在 27 名被试中，YY 条件下的平均反应时（M = 1028.29 ms, SD = 199.72）与 KJ 条件（M = 1039.85 ms, SD = 200.85）之间无显著差异，配对样本 t 检验结果为 t(26) = -0.98, p = .336, Cohen's dz = -0.19。平均差值为 -11.57 ms，95% CI [-35.83, 12.69]。差值分布近似正态（Shapiro-Wilk p = .496）。

注意事项:
    - 第12个被试没有行为数据，实际分析被试数为27
    - 结果保存到../../behavior_results/run7_action_time.csv
