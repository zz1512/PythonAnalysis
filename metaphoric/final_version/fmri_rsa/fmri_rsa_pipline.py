...
def load_roi_masks(config):
    ...
    for roi_file in roi_files:
        ...
        log(f"??ROI mask: {roi_name}", config)
...
def load_lss_trial_data(subject, run, config):
    ...
    if lss_dir is None:
        log(f"???{subject} run-{run}?LSS??", config)
        return None, None

    trial_info_file = lss_dir / "trial_info.csv"
    if not trial_info_file.exists():
        log(f"???{subject} run-{run}?trial_info.csv", config)
        return None, None
    ...
    for _, trial in trial_info.iterrows():
        ...
        row = trial.copy()
        row['beta_file'] = beta_path.name
        row['beta_path'] = str(beta_path)
        valid_rows.append(row)

    if not beta_files:
        log(f"??{subject} run-{run}????beta??", config)
        return None, None

    valid_trial_info = pd.DataFrame(valid_rows).reset_index(drop=True)
    return valid_trial_info, beta_files
