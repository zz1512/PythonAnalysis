...
- P11.4 / P11.5 只产表，不生成 `figures_main/`；**不得**与 `$primaryRois` 合并到同一 FDR family。
...
- **B1**：**必做** — hippocampal-binding subfamily 汇总。
- **C1**：**gated** — 只有当上游 gate 通过才跑。
...
### P12.B1 Hippocampal-binding subfamily 汇总（必做，仅汇总）
...
- `$paper/qc/hippocampal_binding_summary_{roi_tag}/{step5c,edge,word_level_profile,learning_profile,trajectory_compatibility,reliability,retrieval}_subfamily_fdr.tsv`（subfamily 内重做 BH-FDR）
...
依赖提醒：B1 依赖 Step5C + edge + retrieval + 本轮 split profiles + reliability 结果文件都已存在；脚本本身不重算任何 RSA。

### P12.C1 HPC ↔ vmPFC post-encoding connectivity（gated by G2）
...
- P12.B1 只读 primary ROI 主 RSA 产物与 pattern_root。
- P12.C1 依赖已有的 `$paper/qc/gppi_results_*/gppi_group_summary.tsv`（Gate G2），与 B1 互相独立。
- A1 的 SI sensitivity 与 B1/C1 之间**不共享 BH-FDR family**；论文 writeup 里分两处报告。
- 所有 P12 脚本**不修改** `stack_patterns.py` / LSS 产物 / P1–P11 已有 RSA 结果；仅新增 `$paper/qc/<新目录>/` 与 `$paper/tables_main/tables_si/` 下的新表。

## 13. 已撤下与转向

- 已撤下 ERS / cross-phase reinstatement / P13 / P14 相关重跑项，不再作为主线执行或写入主结果。
- 新主线改为 item-level predictive mechanism：learning-stage item trace -> post-stage edge differentiation -> run7 memory。
- 新执行方案见 `metaphoric/final_version/learning_post_memory_prediction_spec.md`。

## 14. Mixed-effects robustness layer（新增，非覆盖）

目的：按 `metaphoric/final_version/mixed_effects_model_spec.md`，用 subject + condition-specific item 随机截距重估关键 item-level 结果，避免 YY/KJ 数字 id 假配对，并保留 run7 缺失与 item 不平衡结构。

```powershell
$py = "C:\Users\dell\anaconda3\envs\PythonAnalysis\python.exe"

& $py metaphoric/final_version/brain_behavior/mixed_effects/run_mixed_effects.py
```

产物：

- `$paper/qc/mixed_effects/mixed_model_table_qc.tsv`
- `$paper/qc/mixed_effects/mixed_effects_roi_models.tsv`
- `$paper/qc/mixed_effects/mixed_effects_trajectory_models.tsv`
- `$paper/qc/mixed_effects/mixed_effects_network_models.tsv`
- `$paper/qc/mixed_effects/mixed_effects_convergence.tsv`
- `$paper/qc/mixed_effects/mixed_effects_memory_logistic_qc.tsv`
- `$paper/qc/mixed_effects/mixed_effects_review.md`

解释边界：

- 主随机结构为 `(1 | subject) + (1 | condition_item_id)`。
- `condition_item_id = condition + "_" + original_pair_id`，YY/KJ 不按数字 id 配对。
- `pre_pair_similarity_z` 使用脑 pattern 的 pre-stage pair similarity，不是 BERT 或词标签。
- memory 当前为 `0/0.5/1`，不是二元变量，因此 M6 logistic mixed model 按 stop condition 跳过。
- mixed-effects 结果作为 hierarchy-aware robustness / exploratory bridge，不直接替代已有 permutation、MVPA 或主 RSA 结果。
